...
ACarPawn::ACarPawn()
{
    ...
    camera_front_center_base_->SetRelativeLocation(FVector(100, 0, 100)); //center
...
void ACarPawn::initializeForBeginPlay(bool engine_sound)
{
    ...
    FTransform VR_Offset_Transform(FVector(0.0f, -5.0f, -335.0f)); //@msihub
    camera_driver_ = this->GetWorld()->SpawnActor<APIPCamera>(pip_camera_class_, camera_transform + VR_Offset_Transform, camera_spawn_params);
    camera_driver_->AttachToComponent(camera_driver_base_, FAttachmentTransformRules::KeepRelativeTransform);
    camera_driver_->enableDoNotOverrideFocusMethod(true);
    //auto camera_driver_info = camera_driver_->getCameraInfo();
    //std::cout << camera_driver_info.fov << std::endl;
    ...
    camera_back_center_->enableDoNotOverrideFocusMethod(true);
    setupInputBindings();
}
...
void ACarPawn::forceFeedbackControlSetup()
{
    APlayerController* controller = this->GetWorld()->GetFirstPlayerController();
    controller->ClientPlayForceFeedback(FSteering_Feedback, false, "ff");
}
...
void ACarPawn::setupInputBindings()
{
    ...
    ///*OCULUS CONTROL SETUP*/
    //UAirBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("OculusLeftReverce", EKeys::OculusTouch_Left_Thumbstick_Y, -1), this, this, &ACarPawn::onMoveForward);
    //UAirBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("OculusLeftAccelerater", EKeys::OculusTouch_Left_Thumbstick_Y, 1), this, this, &ACarPawn::onMoveForward);
    //UAirBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("OculusLeftRight", EKeys::OculusTouch_Left_Thumbstick_X, -0.5), this, this, &ACarPawn::onMoveRight);
    //UAirBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("OculusLeftLeft", EKeys::OculusTouch_Left_Thumbstick_X, 0.5), this, this, &ACarPawn::onMoveRight);
    //UAirBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("Footbrake", EKeys::OculusTouch_Left_Grip_Click, 1), this, this, &ACarPawn::onFootBrake);

    /*ThrustMaster CONTROL SETUP*/
    //UAirBlueprintLib::BindAxisToKey(FName("TMGasPedal"), this, this, &ACarPawn::onGasPedalInput);
    //UAirBlueprintLib::BindAxisToKey(FName("TMFootbrake"), this, this, &ACarPawn::onFootbrakeInput);
    //UAirBlueprintLib::BindAxisToKey(FName("TMSteeringWheel"), this, this, &ACarPawn::onSteeringWheelInput);

    //UAirBlueprintLib::BindActionToKey(FName("TMButtonBReverseGear"), this, &ACarPawn::onReversePressed, true);
    //UAirBlueprintLib::BindActionToKey(FName("TMButtonADriveGear"), this, &ACarPawn::onReverseReleased, true);
    //UAirBlueprintLib::BindActionToKey(FName("TMHandbrake"), this, &ACarPawn::onHandbrakePressed, true);
    //UAirBlueprintLib::BindActionToKey(FName("TMHandbrake"), this, &ACarPawn::onHandbrakeReleased, false);
...
void ACarPawn::onGasPedalInput(float Val)
{
    keyboard_controls_.throttle = mapRangeClamped(Val, 1.0, 0.0, 0.0, 1.0); //Map 1->0 to 0->1  
}

void ACarPawn::onFootbrakeInput(float Val)
{
    keyboard_controls_.brake = mapRangeClamped(Val, 1.0, 0.0, 0.0, 1.0); //Map 1->0 to 0->1
}

void ACarPawn::onSteeringWheelInput(float Val)
{
    keyboard_controls_.steering = mapRangeClamped(Val, 0.0, 1.0, -1.0, 1.0); //Map 0->1 to -1->1
    steering_input1 = mapRangeClamped(Val, 0.0, 1.0, -270.0, 270.0); //Map 0->1 to -1->1 // FOR UPDATING THE VISUAL STEERING WHEEL BONE ANGLE
}
